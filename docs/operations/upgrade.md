...
Each install or upgrade runs the migration in two jobs that take turns on a database lock: `greptile-db-migration-<revision>` runs alongside the rollout, and the `greptile-db-migration-<revision>-hook` hook is what Helm waits for. The second to run finds nothing to apply, and a failed migration fails the release. The failure is in the logs of one of the two jobs (`helm history greptile` shows the revision); failed jobs are kept for a day. A failed migration job means the new version did not finish applying schema changes; fix connectivity or credentials before retrying the upgrade.
