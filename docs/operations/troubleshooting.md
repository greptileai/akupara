...
| `greptile-hydra` stuck in `Init` on a first install | It waits for the first database migration; check the `greptile-db-migration-1` and `greptile-db-migration-1-hook` job logs |
