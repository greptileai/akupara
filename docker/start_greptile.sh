...
# Populate LITELLM_MASTER_KEY if not already set or empty
if ! grep -q "^LITELLM_MASTER_KEY=.\+" .env; then
    echo "Generating LITELLM_MASTER_KEY..."
    random_string=$(generate_random_string)
    if grep -q "^LITELLM_MASTER_KEY=" .env; then
        # Update existing empty LITELLM_MASTER_KEY
        sed -i.bak "s/^LITELLM_MASTER_KEY=.*/LITELLM_MASTER_KEY=$random_string # Auto-generated by Greptile/" .env
    else
        # Add LITELLM_MASTER_KEY if it doesn't exist
        echo "LITELLM_MASTER_KEY=$random_string # Auto-generated by Greptile" >> .env
    fi
fi
